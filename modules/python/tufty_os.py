...
def get_battery_level():
    return 0
    # Battery measurement
    vbat_adc = machine.ADC(tufty2350.PIN_BATTERY)
    vref_adc = machine.ADC(tufty2350.PIN_1V2_REF)
    vref_en = machine.Pin(tufty2350.PIN_VREF_POWER)
...
def launch(file):
    ...
    button_a = machine.Pin(tufty2350.BUTTON_A, machine.Pin.IN, machine.Pin.PULL_DOWN)
    button_c = machine.Pin(tufty2350.BUTTON_C, machine.Pin.IN, machine.Pin.PULL_DOWN)
...
def warning(display, message, width=tufty2350.WIDTH - 20, height=tufty2350.HEIGHT - 20, line_spacing=20, text_size=0.6):
    print(message)
    ...
    display.set_pen(12)
    display.rectangle((tufty2350.WIDTH - width) // 2, (tufty2350.HEIGHT - height) // 2, width, height)
    ...
    for i in range(num_lines):
        ...
        display.text(lines[i], (tufty2350.WIDTH - length) // 2, (tufty2350.HEIGHT // 2) + current_line, tufty2350.WIDTH, text_size)
